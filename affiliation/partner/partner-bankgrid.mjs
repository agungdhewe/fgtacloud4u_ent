...
const btn_removechecked  = $('#pnl_editbankgrid-removechecked')
const btn_addrow = $('#pnl_editbankgrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/bank-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbankform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbankform', () => {
		$ui.getPages().ITEMS['pnl_editbankform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/bank-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbankform', ()=>{
		$ui.getPages().ITEMS['pnl_editbankform'].handler.createnew(header_data)
	})	
}