...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.doc_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editauthform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editauthform', () => {
		$ui.getPages().ITEMS['pnl_editauthform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/auth-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editauthform', ()=>{
		$ui.getPages().ITEMS['pnl_editauthform'].handler.createnew(header_data)
	})	
}